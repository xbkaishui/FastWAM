        ...
        while True:
            try:
                ...
                action = {
                    ...
                    "success_proba": action["success_proba"].cpu().numpy(),
                }
...
if msgpack is not None:
    ...
    # msgpack 缺失时，仍允许模块被 import（避免评测脚本导入阶段直接挂掉）
    def _missing_msgpack(*_args, **_kwargs):
        raise ModuleNotFoundError(
            "msgpack is required for utils.network_utils.WebsocketClientPolicy/WebsocketPolicyServer. "
            "Please install msgpack in the Python environment used to run eval."
        )

    class _MissingPacker:
        def __init__(self, *_args, **_kwargs) -> None:
            _missing_msgpack()

    Packer = _MissingPacker  # type: ignore
    packb = _missing_msgpack  # type: ignore
    Unpacker = _MissingPacker  # type: ignore
    unpackb = _missing_msgpack  # type: ignore
